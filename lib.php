...
/*
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                if ($grade->is_overridden()) {
                    ...
                    $itemcell->attributes['aria-label'] = get_string('overriddengrade', 'gradereport_gradest');
                }
                ...
                if ($item->needsupdate) {
                    ...
                    if ($item->scaleid && !empty($scalesarray[$item->scaleid])) {
                        ...
                        if ($quickgrading and $grade->is_editable()) {
                            ...
                            $itemcell->text .= html_writer::label(
                                get_string('useractivitygrade', 'gradereport_gradest', $gradelabel), $attributes['id'], false,
                        ...
                        if ($quickgrading and $grade->is_editable()) {
                            ...
                            $itemcell->text .= '<label class="accesshide" for="grade_'.$userid.'_'.$item->id.'">'
                                          .get_string('useractivitygrade', 'gradereport_gradest', $gradelabel).'</label>';
                    ...
                    if ($showquickfeedback and $grade->is_editable()) {
                        ...
                        $itemcell->text .= '<label class="accesshide" for="feedback_'.$userid.'_'.$item->id.'">'
                                      .get_string('useractivityfeedback', 'gradereport_gradest', $feedbacklabel).'</label>';
        ...
        $module = array(
            'name'      => 'gradereport_gradest',
            'fullpath'  => '/grade/report/gradest/module.js',
            'requires'  => array('base', 'dom', 'event', 'event-mouseenter', 'event-key', 'io-queue', 'json-parse', 'overlay')
        );
        $PAGE->requires->js_init_call('M.gradereport_gradest.init_report', $jsarguments, false, $module);
        $PAGE->requires->strings_for_js(array('addfeedback', 'feedback', 'grade'), 'grades');
        $PAGE->requires->strings_for_js(array('ajaxchoosescale', 'ajaxclicktoclose', 'ajaxerror', 'ajaxfailedupdate', 'ajaxfieldchanged'), 'gradereport_gradest');
        if (!$enableajax && $USER->gradeediting[$this->courseid]) {
            $PAGE->requires->yui_module('moodle-core-formchangechecker',
                    'M.core_formchangechecker.init',
                    array(array(
                        'formid' => 'gradereport_gradest'
                    ))
            );
    ...
    public function get_grade_table($displayaverages = false) {
        ...
        $fulltable->summary = get_string('summarygrader', 'gradereport_gradest');
    ...
    protected static function get_collapsed_preferences($courseid) {
        if ($collapsed = get_user_preferences('grade_report_gradest_collapsed_categories'.$courseid)) {
        ...
        if (($oldcollapsedpref = get_user_preferences('grade_report_gradest_collapsed_categories')) !== null) {
            if ($collapsedall = unserialize_array($oldcollapsedpref)) {
                ...
                if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
                    ...
                    if (!empty($collapsedall['aggregatesonly']) || !empty($collapsedall['gradesonly'])) {
                        set_user_preference('grade_report_gradest_collapsed_categories', serialize($collapsedall));
                    } else {
                        unset_user_preference('grade_report_gradest_collapsed_categories');
                    }
                }
            } else {
    ...
    protected static function set_collapsed_preferences($courseid, $collapsed) {
        ...
        if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
            set_user_preference('grade_report_gradest_collapsed_categories'.$courseid, json_encode($collapsed));
        } else {
            unset_user_preference('grade_report_gradest_collapsed_categories'.$courseid);
        }
    }
    ...
    public static function do_process_action($target, $action, $courseid = null) {
        ...
        if (!$courseid) {
            debugging('Function grade_report_gradest::do_process_action() now requires additional argument courseid',
                DEBUG_DEVELOPER);
